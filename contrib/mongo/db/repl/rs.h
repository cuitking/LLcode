...
namespace mongo
{
	...
	public:
		...
		const ReplSetConfig::MemberCfg& config() const
		{
			return _config;
		}
		ReplSetConfig::MemberCfg& configw()
		{
			return _config;
		}
		const HeartbeatInfo& hbinfo() const
		{
			return _hbinfo;
		}
		HeartbeatInfo& get_hbinfo()
		{
			return _hbinfo;
		}
		string lhb() const
		{
			return _hbinfo.lastHeartbeatMsg;
		}
		MemberState state() const
		{
			return _hbinfo.hbstate;
		}
		const HostAndPort& h() const
		{
			return _h;
		}
		unsigned id() const
		...
		{
			return _config.potentiallyHot();    // not arbiter, not priority 0
		}
	...
	public:
		...
		{
			change(MemberState::RS_PRIMARY, self);
		}
	...
	*/
	...
	public:
		 ...
		 * node to take over.
		 */
		void addToElectable(const unsigned m)
		{
			lock lk(this);
			_electableSet.insert(m);
		}
		void rmFromElectable(const unsigned m)
		{
			lock lk(this);
			_electableSet.erase(m);
		}
		bool iAmElectable()
		{
			lock lk(this);
	...
	protected:
		...
		string name() const
		{
			return _name;    /* @return replica set's logical name */
		}
		MemberState state() const
		{
			return box.getState();
	...
	private:
		...
		const ReplSetConfig::MemberCfg& myConfig() const
		{
			return _config;
		}
		bool iAmArbiterOnly() const
		{
			return myConfig().arbiterOnly;
		}
	...
	public:
		...
		MemberState state() const
		{
			return ReplSetImpl::state();
		}
		string name() const
		{
			return ReplSetImpl::name();
		}
		const ReplSetConfig& config()
		{
			return ReplSetImpl::config();
		}
		void getOplogDiagsAsHtml(unsigned server_id, stringstream& ss) const
		{
			_getOplogDiagsAsHtml(server_id, ss);
		}
		...
		bool lockedByMe()
		{
			return RSBase::lockedByMe();
		}

		// heartbeat msg to send to others; descriptive diagnostic info
		string hbmsg() const
		{
			if (time(0) - _hbmsgTime > 120)
			{
				return "";
			}
			return _hbmsg;
