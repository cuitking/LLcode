...
namespace mongo
{
	...
	public:
		...
		const FieldRangeSet& multikeyFrs() const
		{
			return _frsMulti;
		}

		bool mustAssertOnYieldFailure() const
		{
			return _mustAssertOnYieldFailure;
		}
	...
	public:
		...
		{
			massert(13335, "yield not supported", false);
			return false;
		}
		/** Recover once the db mutex is regained. */
		virtual void recoverFromYield()
		{
			massert(13336, "yield not supported", false);
		}
	...
	public:
		...
		{
			return _usingPrerecordedPlan;
		}
		/** @return a single plan that may work well for the specified query. */
		QueryPlanPtr getBestGuess() const;

		//for testing
		const FieldRangeSetPair& frsp() const
		{
			return *_frsp;
		}
		const FieldRangeSetPair* originalFrsp() const
		{
			return _originalFrsp.get();
		}
	...
	public:
		...
		void initialOp(const shared_ptr<QueryOp>& originalOp)
		{
			_baseOp = originalOp;
		}
		...
		{
			return _or ? (!_tableScanned && !_org->orFinished()) : _i == 0;
		}
		/** @return non-$or version of explain output. */
		BSONObj oldExplain() const
		{
			assertNotOr();
			return _currentQps->explain();
		}
		...
		bool modifiedKeys() const
		{
			return _currentQps->modifiedKeys();
		}
		bool hasMultiKey() const
		{
			return _currentQps->hasMultiKey();
		}
	...
	public:
		...
		{
			return _c->supportYields();
		}
		virtual BSONObj indexKeyPattern()
		{
			return _c->indexKeyPattern();
		}
		...
		{
			return _mps->modifiedKeys();
		}

		virtual bool isMultiKey() const
		{
			return _mps->hasMultiKey();
		}
		...
		{
			return _nscanned >= 0 ? _nscanned + _c->nscanned() : _nscanned;
		}
		/** just for testing */
	...
	private:
		...
		public:
			...
			virtual shared_ptr<Cursor> newCursor() const
			{
				return qp().newCursor();
			}
			virtual long long nscanned()
