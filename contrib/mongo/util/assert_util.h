...
namespace mongo
{
	...
	*/
	...
	inline void uasserted(int msgid , string msg)
	{
	...
	*/
	...
	inline void msgassertedNoTrace(int msgid, const string& msg)
	{
		msgassertedNoTrace(msgid , msg.c_str());
	}
	void msgasserted(int msgid, const char* msg) MONGO_NORETURN;
	inline void msgasserted(int msgid, string msg)
	{
		msgasserted(msgid, msg.c_str());
	}

	/* convert various types of exceptions to strings */
	inline string causedBy(const char* e)
	{
		return (string)" :: caused by :: " + e;
	}
	inline string causedBy(const DBException& e)
	{
		return causedBy(e.toString().c_str());
	}
	inline string causedBy(const std::exception& e)
	{
		return causedBy(e.what());
	}
	inline string causedBy(const string& e)
	{
	...
	inline void verify(int msgid , bool testOK)
	{
		if (! testOK)
		{
			verifyFailed(msgid);
