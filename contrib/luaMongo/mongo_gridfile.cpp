...
namespace
{
	inline mongo::GridFile* userdata_to_gridfile(lua_State* L, int index)
	{
		void* ud = 0;

		ud = luaL_checkudata(L, index, LUAMONGO_GRIDFILE);
		mongo::GridFile* gridfile = *((mongo::GridFile**)ud);

		return gridfile;
	}
}

int gridfile_create(lua_State* L, mongo::GridFile gf)
{
	mongo::GridFile** gridfile = (mongo::GridFile**)lua_newuserdata(L, sizeof(mongo::GridFile**));

	*gridfile = new mongo::GridFile(gf);
...
{
	mongo::GridFile* gridfile = userdata_to_gridfile(L, 1);
...
static int gridfile_field(lua_State* L)
{
	...
	mongo::BSONElement elem = gridfile->getFileField(field_name);
...
{
	mongo::GridFile* gridfile = userdata_to_gridfile(L, 1);
	mongo::Date_t upload_date = gridfile->getUploadDate();
