...
extern int gridfile_create(lua_State* L, mongo::GridFile gf);

namespace
{
	inline mongo::GridFS* userdata_to_gridfs(lua_State* L, int index)
	{
		void* ud = 0;
		ud = luaL_checkudata(L, index, LUAMONGO_GRIDFS);
		mongo::GridFS* gridfs = *((mongo::GridFS**)ud);
		return gridfs;
	}
}
...
static int gridfs_new(lua_State* L)
{
	...
	try
	{
		...
		mongo::GridFS** gridfs = (mongo::GridFS**)lua_newuserdata(L, sizeof(mongo::GridFS*));

		if (n >= 3)
		{
			const char* prefix = luaL_checkstring(L, 3);

			*gridfs = new mongo::GridFS(*connection, dbname, prefix);
		}
		else
		{
			*gridfs = new mongo::GridFS(*connection, dbname);
		}
...
{
	mongo::GridFS* gridfs = userdata_to_gridfs(L, 1);
	int resultcount = 1;

	try
	{
		mongo::GridFile gridfile = gridfs->findFile(luaL_checkstring(L, 2));
...
static int gridfs_list(lua_State* L)
{
	...
	std::auto_ptr<mongo::DBClientCursor> autocursor = gridfs->list();
...
static int gridfs_store_file(lua_State* L)
{
	...
	try
	{
		mongo::BSONObj res = gridfs->storeFile(filename, remote, content_type);
		bson_to_lua(L, res);
	}
	catch (std::exception& e)
	{
...
static int gridfs_store_data(lua_State* L)
{
	...
	try
	{
		mongo::BSONObj res = gridfs->storeFile(data, length, remote, content_type);
		bson_to_lua(L, res);
	}
	catch (std::exception& e)
	{
